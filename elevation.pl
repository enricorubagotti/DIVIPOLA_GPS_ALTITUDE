...
while (my $line=<input>)
	...
	my $commandAltitude='curl -H "Accept: application/json" -X GET    https://maps.googleapis.com/maps/api/elevation/json?locations='.$line[7].'%2C'.$line[8].'\&key=<google_key>  | grep  elevation   ';
